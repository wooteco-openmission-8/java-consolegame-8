...
- [ ] 타일 값별 enum 상수 정의 (EMPTY, TWO, FOUR, ... TWO_THOUSAND_FORTY_EIGHT)
- [ ] 각 enum 상수에 값(int value) 저장
- [ ] 각 enum 상수에 배경색(Color backgroundColor) 저장
- [ ] 각 enum 상수에 텍스트 색상(Color textColor) 저장
- [ ] 숫자 값으로 TileType 찾는 메서드 `fromValue(int number)` 구현
- [ ] 배경색 반환 메서드 `getBackgroundColor()` 구현
- [ ] 텍스트 색상 반환 메서드 `getTextColor()` 구현
